...
def test_connect(sid, environ):
    sio.emit('my response', {'data': 'Connected', 'count': -1}, room=sid,
             namespace='/test')
...
if __name__ == '__main__':
    if sio.async_mode == 'threading':
        ...
        try:
            ...
            websocket = False
        if websocket:
            pywsgi.WSGIServer(('', 4999), app,
                              handler_class=WebSocketHandler).serve_forever()
        else:
            pywsgi.WSGIServer(('', 4999), app).serve_forever()
    elif sio.async_mode == 'gevent_uwsgi':
        print('Start the application through the uwsgi server. Example:')
        print('uwsgi --http :4999 --gevent 1000 --http-websockets --master '
